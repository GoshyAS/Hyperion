...
_actions = 
	...
	while {(_unit getvariable "action")=="true"} do
		...
		if ((_unit getvariable "order") == "nil") then
			{
				_whistle = _unit addAction ["Sifler !", _dogWhistle, [_unit, _dogGrowl,_playSound,_dogReturnIdle,_dogVehicle]];
		...
		if ((_unit getvariable "order") == "idle") then
			{
				_follow = _unit addAction ["<t color = '#ffff00'>Suivre</t>", _dogFollow, [_unit,_playSound]];
				_find = _unit addAction ["<t color = '#ffff00'>Chercher</t>", _dogSeek, [_unit,_playSound]];
				_rest = _unit addAction ["<t color = '#ff0000'>Cacher !</t>", _dogHide, [_unit,_playSound]];
				_heel = _unit addAction ["<t color = '#ffff00'>Assis</t>", _dogHeel, [_unit,_playSound]];
				_unit setvariable ["step","wait"];
