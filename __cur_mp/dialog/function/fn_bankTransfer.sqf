...
if(_val > life_hypatmcash) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > life_hypatmcash) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};

life_hypatmcash = life_hypatmcash - (_val + _tax);
