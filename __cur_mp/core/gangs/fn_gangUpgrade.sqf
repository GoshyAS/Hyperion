...
if(_action) then {
	if(life_hypatmcash < _upgradePrice) exitWith {
		hint parseText format[
			...
			[life_hypatmcash] call life_fnc_numberText,
			[_upgradePrice - life_hypatmcash] call life_fnc_numberText
		];
	};
	__SUB__(life_hypatmcash,_upgradePrice);
