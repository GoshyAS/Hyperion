...
if(count life_houses >= ((call life_houseLimit))) exitWith {hint format[localize "STR_House_Max_House3",(call life_houseLimit)]};
...
if(_action) then {
	if(life_hypatmcash < (_houseCfg select 0)) exitWith {hint format [localize "STR_House_NotEnough"]};
	...
	life_hypatmcash = life_hypatmcash - (_houseCfg select 0);
