...
switch (_code) do
{
	...
	case 24:
	{
	  ...
	  if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,independent]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
	  {
	   if([false,"zipties",1] call life_fnc_handleInv) then
	    {
	    [] call life_fnc_restrainAction;
	    hint "La victime est attaché, utilisez le menu d'action pour plus d'information";
	   }
	   else
	   {
	    hint "Vous n'avez pas de liens !";
	   };
	  };
	};
	...
	//Using Player Menu to block long range view
	case 41:
	{
		if(!dialog) then
		{
		...
		if(!_alt && !_ctrlKey) then {
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					...
					hint parseText format ["<t color='#27ae60'><t size='1'><t align='center'>Véhicule ouvert</t></t></t>"];
					} else {
						if(local _veh) then {
							_veh lock 2;
						} else {
					...
					hint parseText format ["<t color='#c0392b'><t size='1'><t align='center'>Véhicule fermé</t></t></t>"];
					};
				};
			};
		};
	...
	case 67:
	{
		...
		if (soundVolume < 0.21) then 
			...
			{ 0 fadeSound 0.2; };
		systemChat format["Vous avez mis des bouchons d'oreilles, vous entendez désormais à %1%2.", soundVolume * 100, "%"];
		_handled = true;
	};
};
