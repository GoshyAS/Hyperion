...
while{true} do 
{
	...
	if(isEngineOn _vehicleToFuel && ((driver _vehicleToFuel) == player) && !(_vehicleToFuel isKindOf "Air") && (_vehicleToFuel != player) ) then
	{
		...
		{
			hint "Votre réservoir est vide. Dépêchez-vous d'aller à la station-service la plus proche !";
		}
		else
		{
			if(fuel _vehicleToFuel < 0.03) then
			{
				hint "J'espère que vous pouvez courir,parce que vous avez manqué de d'essence !";
			};
		};
	};
	sleep 2;
