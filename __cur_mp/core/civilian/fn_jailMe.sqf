...
if(_time <= 0) then { _time = time + (15 * 60); hintC "Signaler à l'administration: JAIL_FALLBACK_15, time is zero!"; };
...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		{
			hintSilent format["Temps restant:\n %1\n\nPayer la caution: %3\nMontant de la caution: $%2",_countDown,[life_bail_amount] call life_fnc_numberText];
		}
		else
		{
