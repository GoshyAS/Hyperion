...
if(life_hypcash < _betamt) exitWith {hint format["Vous n'avez pas asser d'argent pour jouer (%1$)",_betamt];};
life_hypcash = life_hypcash - _betamt;
...
{ 
	_win = 4 * _betamt;
	life_hypcash = life_hypcash + _win;
...
else
{
	...
	if( _number == 13 ) then
	{
		...
		{		
			...
			_winnings = _display displayCtrl 6001;
			_winnings ctrlSetText "Vous Perdez. Le Dealer a obtenu le Black Jack.";
			_bet1 ctrlEnable true;
			_bet2 ctrlEnable true;
			_bet3 ctrlEnable true;
			_bet4 ctrlEnable true;
